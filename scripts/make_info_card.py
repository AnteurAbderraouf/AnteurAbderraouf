...
ROWS = [
    ("host",),
    ("kv", "Now", "Freelance developping software for clients @ remote"),
    ("kv", "Also", "Cybersecurity and AI enthusiast; History & Philosophy"),
    ("kv", "Edu", "Master Computer Science, USTHB '26"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Frontend",   "React, Vite, Electron, React Router"),
    ("kv", "Backend",    "Laravel, PHP 8.2, PostgreSQL, Redis"),
    ("kv", "Auth / Sécu","Sanctum, TOTP 2FA, bcrypt, rate limiting"),
    ("kv", "Cloud",      "Hetzner, Nginx, Cloudflare, Let's Encrypt"),
    ("gap",),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">Das Stein Adler@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "gap":
        ...
        continue
    if kind == "host":
        username = "Das Stein Adler"
        header_w = int((len(username) + len("@github")) * 8.4) + 12
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">{username}</tspan><tspan fill="{MUTED}">@</tspan>'
                 f'<tspan fill="{ACCENT}">github</tspan></text>'
                 f'<line x1="{KEY_X + header_w}" y1="{y-4:.1f}" x2="{W-PAD}" y2="{y-4:.1f}" '
