...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">antara@github: ~$ ./portrait.sh</text>')
...
whoami_name = "Das Stein Adler"
prompt = "antara@github:~$ whoami "
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{prompt}<tspan fill="{INK}">{whoami_name}</tspan></text>')
# ~7.8px/char at font-size 13 monospace; cursor sits at end of the display name
cursor_x = PAD + int((len(prompt) + len(whoami_name)) * 7.8) + 4
